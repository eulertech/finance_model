...
#    print("index located at %d" %index)
    ...
    COI = costOfInsurance(t)    
    insuranceCostToDate.append(COI)
    print("cost of insurance is %f" %COI)       
    insuranceBal = COI
    #deposit array
...
plt.title('Historical S&P 500 Index', fontsize = fsize)
plt.setp(ax1.get_xticklabels(),rotation=45, fontsize=8,visible=False)

ax2 = plt.subplot(222)
ax2.plot(statementDateList,insuranceCostToDate)
ax2.set_ylim([0,max(insuranceCostToDate)])
plt.title('Assumed Monthly Insurance Premium Over Time',fontsize = fsize)     
plt.setp(ax2.get_xticklabels(),rotation=45, fontsize=8,visible=False)

ax3 = plt.subplot(223)
ax3.plot(statementDateList,rateList,label='IUL')
ax3.plot(statementDateList,stockRateList,label='Stock Gap-12')
#ax3.set_ylim([-max(15,max(stockRateList))-5,max(15,max(stockRateList))+5])
...
plt.title('Rate appplied by month (%)',fontsize = fsize)
plt.setp(ax3.get_xticklabels(),rotation=45, fontsize=8)
  
ax4=plt.subplot(224)
ax4.plot_date(statementDateList,balanceArray,'g-', label = 'IUL')
ax4.plot_date(statementDateList,depositToDate,'k-',mew=1,ms=2,linewidth='1',label = 'Principle')
ax4.plot_date(statementDateList,stockReturnArray, 'r-',mew=1,ms=2,linewidth='1', label = 'Stock Return') 
ax4.set_ylim( [int(balanceArray.min()), max(int(balanceArray.max()),depositToDate[-1],max(stockReturnArray))] )
...
for label in legend.get_lines():
    label.set_linewidth(1.0)        
plt.title('Balance,deposit and stock investment return over time ($)',fontsize=fsize)    
plt.setp(ax4.get_xticklabels(),rotation=45, fontsize=8)
plt.show()
picname = str(monthlyDeposit)+'_HindcastRealY01_from_' + str(statementDateList[0])[0:10] + 'to_'+str(statementDateList[-1])[0:10] 
plt.savefig(picname)
